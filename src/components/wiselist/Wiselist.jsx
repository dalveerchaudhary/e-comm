...
import { removeFromWiseList } from '../../features/wiseSlice'; 

const Wiselist = () => {
  const { wiselistItems, totalAmount } = useSelector(state => state.Wiselist);
  const dispatch = useDispatch();

  return (
    <div className="p-4 ">
      <h2 className="text-xl font-bold mb-4">WiseList Bag</h2>
      {wiselistItems.length === 0 ? (
        <p>Your wiselist is empty!</p>
      ) : (
        <div className='flex  flex-wrap justify-center gap-4'>
          {wiselistItems.map(item => (
            <div key={item.id} className=" p-4 mb-2 border rounded-md shadow-md  w-[300px]">
              ...
              <img src={item.image} alt="" className="w-[296px] h-[296px]"/>
                ...
                <button
                onClick={() => dispatch(removeFromWiseList(item))}
